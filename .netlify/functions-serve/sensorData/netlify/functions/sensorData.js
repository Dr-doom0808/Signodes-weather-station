...
var require_api = __commonJS({
  "src/config/api.cjs"(exports2, module2) {
    var SCRIPT_ID2 = process.env.GOOGLE_SCRIPT_ID || process.env.VITE_GOOGLE_SCRIPT_ID;
    var GOOGLE_SCRIPT_URL2 = SCRIPT_ID2 ? `https://script.google.com/macros/s/${SCRIPT_ID2}/exec` : process.env.GOOGLE_SCRIPT_URL || "";
    var API_BASE = "/api";
    var COLS = {
      datetime: 0,
      pressure: 1,
      ...
      uvRisk: 15,
      mq_co: 16
      // MQ-7 Carbon Monoxide sensor column
    };
    function getUVRisk2(uvIndex) {
    ...
    module2.exports = {
      ...
      GOOGLE_SCRIPT_URL: GOOGLE_SCRIPT_URL2,
      API_BASE,
      COLS,
      getUVRisk: getUVRisk2
    };
  }
});

// netlify/functions/sensorData.js
var axios = require_axios();
var { SCRIPT_ID, getUVRisk } = require_api();
var GOOGLE_SCRIPT_URL = process.env.GOOGLE_SCRIPT_URL || `https://script.google.com/macros/s/${SCRIPT_ID}/exec`;
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (textData.trim().startsWith("[")) {
        ...
        if (Array.isArray(jsonData) && jsonData.length > 0) {
          sensorData = jsonData.map((row, index) => {
            ...
            return {
              ...
              lastUpdated: row[0] || (/* @__PURE__ */ new Date()).toISOString().replace("T", "_").split(".")[0]
            };
      ...
      sensorData = rows.map((row) => {
        ...
        return {
          ...
          lastUpdated: columns[0] || (/* @__PURE__ */ new Date()).toISOString().replace("T", "_").split(".")[0]
        };
      }).filter((item) => item !== null);
    }
    ...
    return {
      ...
      body: textData
      // Return the raw data from Google Apps Script
    };
  } catch (error) {
